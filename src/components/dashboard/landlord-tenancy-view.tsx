      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-xs font-bold uppercase tracking-wider text-gray-700 flex items-center gap-1.5">
            <LuBuilding2 className="w-3.5 h-3.5 text-[#005F56]" />
            Your Residences ({properties.length})
          </h2>
          <span className="text-[11px] text-gray-400">Click any residence to manage its room types &amp; assigned tenants</span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
          {properties.map((p) => {
            ...
            return (
              <Link
                key={p.id}
                href={`/landlord/properties/${p.id}/tenancy`}
                className="group p-4 rounded-2xl border border-gray-200 hover:border-[#005F56] bg-white hover:bg-[#005F56]/5 text-left transition-all cursor-pointer space-y-2.5 shadow-2xs hover:shadow-md"
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="flex-1 truncate">
                    <h3 className="text-xs font-bold truncate text-gray-900 group-hover:text-[#005F56] transition-colors">
                      {p.title}
                    </h3>
                  ...
                  <span className="w-6 h-6 rounded-full bg-slate-100 group-hover:bg-[#005F56] text-slate-500 group-hover:text-white flex items-center justify-center text-xs shrink-0 transition-all">
                ...
                <div className="flex items-center justify-between text-[11px] pt-1.5 border-t border-gray-100">
                  <span className="font-semibold text-gray-600 flex items-center gap-1">
                    <LuUsers className="w-3 h-3 text-[#005F56]" />
                  ...
                  return (
                    <tr key={tenancy.id} className="hover:bg-gray-50/60 transition-colors">
                      ...
                      <td className="py-3.5 px-4">
                        <Link
                          href={`/landlord/properties/${tenancy.propertyId}/tenancy`}
                          className="font-bold text-gray-900 hover:text-[#005F56] transition-colors block"
                        >
                          {tenancy.propertyTitle || "Residence"}
                        </Link>
                        <span className="text-[11px] text-[#005F56] font-semibold block">
